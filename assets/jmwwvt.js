  ...
  exports['default'] = _ember['default'].Component.extend({
    actions: {
      save: function save() {
        ...
        var link = image.get('link').split('');
        for (var i = 0; i < link.length; i++) {
          if (link[i] === '.') {
            var ext = link[i + 1] + link[i + 2] + link[i + 3];
            if (ext === 'png' || ext === 'jpg') {
              this.sendAction('save', this.get('image'));
              return;
            }
...
if (!runningTests) {
<<<<<<< HEAD
  require("jmwwvt/app")["default"].create({"name":"jmwwvt","version":"0.0.0+3eb6c102"});
=======
